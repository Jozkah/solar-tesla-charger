...
test('sums energy from the left sample of each interval', () => {
  // Two 1-hour intervals at 1000 W => 2000 Wh. The third sample only closes
  // the second interval and contributes no energy of its own.
  const rows = [
    sample(d0, { charge_w: 1000, charging: 1 }),
    sample(d0 + 3_600_000, { charge_w: 1000, charging: 1 }),
    sample(d0 + 7_200_000, { charge_w: 0 }),
  ];
  const r = computeDayRollup(rows, d0, d0 + DAY);
  assert.equal(Math.round(r.car_wh), 2000);
...
test('splits car energy into solar and grid shares', () => {
  // 2000 W charge with 500 W imported for an hour => 500 grid, 1500 solar.
  const rows = [
    sample(d0, { charge_w: 2000, import_w: 500, charging: 1 }),
    sample(d0 + 3_600_000, { charge_w: 0 }),
  ];
  const r = computeDayRollup(rows, d0, d0 + DAY);
  assert.equal(Math.round(r.car_grid_wh), 500);
  assert.equal(Math.round(r.car_solar_wh), 1500);
});

test('house consumption is generation plus grid power, floored at zero', () => {
  // Growatt generation reads negative; solax reads positive.
  const rows = [
    sample(d0, { solar2_w: -1000, solax_w: 500, grid_power: -200 }),
    sample(d0 + 3_600_000, {}),
  ];
  const r = computeDayRollup(rows, d0, d0 + DAY);
  assert.equal(Math.round(r.solar2_wh), 1000);
  assert.equal(Math.round(r.solax_wh), 500);
  assert.equal(Math.round(r.used_wh), 1300); // 1500 generated - 200 exported
});
